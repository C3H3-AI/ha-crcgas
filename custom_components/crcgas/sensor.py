...
async def _import_meter_history_to_entity(hass, entry, bill_history, only_missing=False):
    ...
    if not entity_id:
        _LOGGER.warning("燃气表总读数传感器尚未注册，跳过")
        return
    ...
    def _do_inject():
        ...
        try:
            ...
            # === 注入累计燃气费用 ===
            if cost_entity_id and cost_stats_data:
                conn.execute(
                    "INSERT OR IGNORE INTO statistics_meta "
                    "(statistic_id, source, unit_of_measurement, has_mean, has_sum, name) "
                    "VALUES (?, ?, ?, 0, 1, '累计燃气费用')",
                ...
                if cost_row:
                    cost_mid = cost_row[0]
                    conn.execute("DELETE FROM statistics WHERE metadata_id = ?", (cost_mid,))
                    conn.execute("DELETE FROM statistics_short_term WHERE metadata_id = ?", (cost_mid,))
                    cost_rows = [(cost_mid, ts, val, val, val, val, 0.0, ts, now) for ts, val in cost_stats_data]
                    conn.executemany(
                        "INSERT INTO statistics "
                        "(metadata_id, start_ts, state, sum, min, max, mean, last_reset_ts, created_ts) "
                        "VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)",
            ...
            conn.commit()
            _LOGGER.info("历史累计和费用注入完成")
        except sqlite3.Error as e:
            _LOGGER.error("SQLite写入历史累计失败: %s", e)
            if conn:
                conn.rollback()
        except Exception as e:
        ...
        try:
            ...
            if bill_history:
                ...
                try:
                    ...
                    _LOGGER.info("历史统计导入完成")
                except Exception as e2:
                    _LOGGER.warning(f"历史统计导入失败: {e2}")
        except Exception as e:
